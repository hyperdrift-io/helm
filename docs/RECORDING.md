...
Everything is on T1 now: the nervous-system map, the fleet cards (each flips to
stream the crew working on its back), the crew manifest, and the live ledger.
...
Click **Attack it** on the cargo card. Point at what happens:
...
### 1:05 — The crew, and why it's safe (T1)
...
### 1:35 — Break, and the injection (T1)
Scroll back to the cards. Click **Break it** on the sandbox card — watch its
synapse fire and the card flip to stream the crew.
...
### 2:30 — Surge (T1)
Click **Surge traffic** on the cargo card.
...
### 3:00 — Close (T1)
Scroll back to the nervous-system map.
